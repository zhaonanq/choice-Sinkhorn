...
def ilsr(nb_items, rankings, max_iter=int(1e10), eps=1e-8, ground_truth=None):
    ...
    while True:
        j += 1
        prev_strengths = strengths
        strengths = lsr(nb_items, rankings, initial_strengths=strengths)
        if (ground_truth is not None):
            errors.append(np.linalg.norm(strengths-ground_truth))
        #loglik = log_likelihood(rankings, strengths)
        #if abs(loglik - prev_loglik) < eps:
        if max(abs(np.log(strengths) - np.log(prev_strengths))) < eps:
        #if max(abs(strengths - prev_strengths)) < eps:
            return strengths,j,errors
        #prev_loglik = loglik
